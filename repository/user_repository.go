...
func (r *UserRepository) CreateUser(user *entity.User) error {
    ...
    return r.db.QueryRow(query, user.Email, user.PasswordHash).Scan(&user.ID, &user.CreatedAt, &user.UpdatedAt)
}

func (r *UserRepository) GetUserByEmail(email string) (*entity.User, error) {
    var user entity.User
    query := `SELECT id, email, password_hash, created_at, updated_at FROM users WHERE email = $1 AND deleted_at IS NULL`
...
func (r *UserRepository) GetAllUsers() ([]entity.User, error) {
    query := `SELECT id, email, created_at FROM users WHERE deleted_at IS NULL`
...
func (r *UserRepository) UpdateUser(user *entity.User) error {
    ...
    _, err := r.db.Exec(query, user.Email, user.ID)
    return err
}
...
func (r *UserRepository) GetUserByID(id int) (*entity.User, error) {
    var user entity.User
    query := `SELECT id, email, password_hash FROM users WHERE id = $1 AND deleted_at IS NULL`
